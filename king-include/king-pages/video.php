...
//	Process input
	...
	function king_facebook($videocontent) { 
   
		$paths = explode("/",$videocontent);
        $num = count($paths);
        for($i=$num-1; $i > 0; $i--){
            if($paths[$i] != ""){
                $video_id = $paths[$i];
...
//	Prepare content for theme
	...
	if (qa_opt('links_in_new_window')) {
		$vidupload = '<ul class="vidtab" role="tablist">
				...
				<label id="king-vidthumb" class="king-vidthumb" for="checkbox-menu3">Upload Thumbnail</label>
				<input name="content2" id="content" autocomplete="off" type="text" class="hide">
				</div>';
	} else {
